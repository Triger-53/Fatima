...
const AppointmentDetailsStep = ({
	...
	const selectedCenter = offlineLocations.find(
		(loc) => loc.id === formData.medicalCenter
	)
				...
				<div className="mt-6">
					...
					<div className="grid grid-cols-1 md:grid-cols-1 gap-4">
						{offlineLocations.map((center) => (
							<label
								key={center.id}
								className={`cursor-pointer group relative overflow-hidden rounded-2xl border-2 transition-all duration-300 hover:shadow-lg ${
					...
					<p className="text-xs text-gray-500 mt-3 flex items-center">
						...
						Choose the medical center where you'd like to have your in-person
						appointment
					</p>
				</div>
			)}
			...
			<div>
				...
				{loadingSlots ? (
					...
					<div className="w-full px-4 py-3 border-2 border-red-200 rounded-xl bg-red-50 text-red-700 flex items-center">
						...
						No available slots for this date and consultation method. Please
				...
				<p className="text-xs text-gray-500 mt-1 flex items-center">
					...
					{formData.consultationMethod === "online"
						...
						? `Available time slots at ${selectedCenter?.name}`
